...
//                    PubVar.m_DoEvent.mRoundLinePresenter = new RoundGPSLine();
    ...
    private void startNomalPatrol() {
        ...
        final AlertDialog.Builder startRoundDialog = new AlertDialog.Builder(MainActivityBackup.this);
        ...
        final TextView mLon = (TextView) layout.findViewById(R.id.tvLon);
        TextView mLat = (TextView) layout.findViewById(R.id.tvLat);
        TextView mHigh = (TextView) layout.findViewById(R.id.tvHigh);
        if (PubVar.m_GPSLocate != null && PubVar.m_GPSLocate.m_LocationEx != null) {

            try {
                mLon.setText(Tools.ConvertToDigi(PubVar.m_GPSLocate.m_LocationEx.GetGpsLongitude() + "", 7));
                mLat.setText(Tools.ConvertToDigi(PubVar.m_GPSLocate.m_LocationEx.GetGpsLatitude() + "", 7));
                mHigh.setText(PubVar.m_GPSLocate.m_LocationEx.GetGpsAltitude() + "");
            } catch (Exception ex) {
...
//        startRoundDialog.setMessage("您要现在开始新的巡护？");
        startRoundDialog.setPositiveButton("开始", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                String lonstr=mLon.getText().toString().trim();
                String latstr=mLon.getText().toString().trim();
                   if(Double.parseDouble(lonstr)==0&&Double.parseDouble(latstr)==0){
